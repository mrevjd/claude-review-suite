...
# ANCHOR: STRIPE_KEY = "sk_live_EXAMPLE_0000000000000000000000"
...
def export_report(request):
    ...
    # ANCHOR: "/usr/local/bin/report --month=" + month, shell=True
...
def store_password(user_id, password):
    ...
    # ANCHOR: hashlib.md5(password.encode()).hexdigest()
...
def new_reset_token():
    ...
    # ANCHOR: random.choice("0123456789abcdef")
...
def restore_prefs(request):
    ...
    # ANCHOR: pickle.loads(request.body)
...
def fetch_avatar(request):
    ...
    # trust boundary.
    # ANCHOR: requests.get(url, timeout=5)
...
def login(request):
    ...
    # enumeration oracle.
    # ANCHOR: return {"error": f"no account for {email}"}, 404
    if row is None:
...
def get_order(request, order_id):
    try:
        ...
        # ANCHOR: "trace": traceback.format_exc(),
        return {
            "error": str(exc),
